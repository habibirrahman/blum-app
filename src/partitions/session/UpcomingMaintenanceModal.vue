  ...
  <div class="flex fixed top-0 left-0 z-50 flex-col w-screen h-screen bg-slate-1 p-safe">
    <div class="flex flex-col h-full bg-white">
      ...
      <div class="flex justify-between items-center px-4 h-14 shrink-0">
        <div class="text-lg font-semibold text-slate-10">Upcoming maintenance targets</div>
        <button @click="emit('close')" class="flex justify-center items-center w-8 h-8 rounded-full hover:bg-slate-2">
      ...
      <div class="overflow-y-auto flex-1">
        <div class="px-4 pb-4 text-sm text-slate-8">
          Select which targets to maintain today. The next maintenance date for selected targets will be rescheduled automatically.
        </div>
          ...
          <div class="flex justify-between items-center px-4 py-3 w-full bg-white">
            <div class="flex flex-col gap-1 pr-4">
              <div class="flex gap-2 items-center">
                ...
                <div class="text-xs font-medium text-slate-6 truncate max-w-[150px]">
                  {{ target.curriculum_name }}
                </div>
              </div>
              <div class="text-sm font-bold text-slate-9">{{ target.name }}</div>
              <div class="text-sm text-slate-6 line-clamp-2">
                {{ target.description || '' }}
              </div>
              <div v-if="target.maintenance_next_date" class="flex gap-1 items-center mt-1 text-xs font-medium text-info-6">
      ...
      <div class="fixed bottom-0 left-0 p-4 w-full bg-white border-t border-slate-3 pb-safe">
        <div class="h-[130px] flex flex-col gap-5">
          ...
          <AppButton 
            ...
            @click="onSubmit"
          >
            Run maintenance for {{ selectedIds.length }} target(s)
          </AppButton>
        </div>
      </div>
    </div>
